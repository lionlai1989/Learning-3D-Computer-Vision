...
def get_pixels_from_image(image_size, camera):
    ...
    x = torch.arange(start=0, end=W, step=1)  # [start, end), dtype=torch.float32
    y = torch.arange(start=0, end=H, step=1)  # [start, end), dtype=torch.float32

    # Convert to the range [-1, 1] in both x and y
    # TODO: It doesn't include exact +1.
    x = x * 2 / W - 1
    y = y * 2 / H - 1
...
def get_random_pixels_from_image(n_pixels, image_size, camera):
    ...
    num_rays, n_channels = xy_grid.shape

    # Random subsampling of pixel coordinaters
    indices = torch.randperm(num_rays)
    selected_indices = indices[:n_pixels]
    xy_grid_sub = xy_grid[selected_indices, ...]
    return xy_grid_sub
...
def get_rays_from_pixels(xy_grid, image_size, camera):
    ...
    ndc_points = torch.cat([xy_grid, torch.ones_like(xy_grid[..., -1:])], dim=-1).to(
        camera.device
    )  # (num_rays, 3)

    # Use camera.unproject to get world space points from NDC space points
    ...
    # Get ray directions as image_plane_points - rays_o
